...
const[submitLoder,setSubmitLoader]=useState<boolean>(false)
  ...
  const defaultValues: HomePurchaseType = storedData
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<HomePurchaseType>({
    resolver: zodResolver(HomePurchase),
    mode: "onTouched",
    defaultValues,
  });
  ...
  useEffect(() => {
    ...
    const fetchSteps = async () => {
      try {
        const res = await fetch("/LoanFormsData/HomePurchaseData.json");
    ...
    switch (field.type) {
      ...
      case "phone":
        return (
          <div
            key={name}
            className="flex flex-col mb-8 relative w-full items-center mx-auto  sm:px-4"
          >
        ...
        <div className="w-full max-w-xl  p-6 md:p-8 rounded-lg  z-10">
          ...
          <form onSubmit={handleSubmit(onSubmit, onError)}>
            <div className="flex justify-center items-center mb-5">
              <h1 className="text-[16px] text-green-800">HOME PURCHASE LOAN</h1>
            </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between mt-15 gap-3">
                ...
                {currentStep < steps.length - 1 && (
                  <button
                    type="button"
                    onClick={nextStep}
                    className="w-full sm:w-auto mt-2 sm:mt-0 px-5 py-3 sm:px-8 sm:py-3 text-[17px] bg-green-600 hover:bg-green-700 text-white rounded-lg transition ease-in-out duration-200 cursor-pointer"
                  >
                    Next →
                  </button>
                )}
